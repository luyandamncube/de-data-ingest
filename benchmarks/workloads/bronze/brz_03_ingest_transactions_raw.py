"""Benchmark workload for BRZ_03 transactions Bronze ingestion."""
...
def run(
    *,
    ...
    """Read transactions.jsonl, add a run-level timestamp, and write Delta."""
    ...
    transactions_df = spark.read.schema(BRONZE_TRANSACTIONS_SCHEMA).json(
        str(transactions_path)
    )
    transactions_with_ts = transactions_df.withColumn(
        "ingestion_timestamp",
    ...
    (
        transactions_with_ts.coalesce(1)
        .write.format("delta")
        .mode("overwrite")
    ...
    return BenchmarkExecution(
        ...
        notes=(
            "PySpark Bronze transactions ingest benchmark"
        ...
        artifacts={
            ...
            "implementation_ref": (
                "benchmarks.workloads.bronze.brz_03_ingest_transactions_raw:run"
            ),
        },
    )
