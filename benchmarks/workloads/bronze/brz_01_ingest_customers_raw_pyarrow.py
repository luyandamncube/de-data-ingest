"""PyArrow benchmark workload for BRZ_01 customers Bronze ingestion."""
...
def run(
    *,
    ...
    customers_table = csv.read_csv(
        customers_path,
        read_options=csv.ReadOptions(use_threads=True),
        parse_options=csv.ParseOptions(delimiter=","),
        convert_options=csv.ConvertOptions(column_types=_arrow_column_types()),
    )
    ingestion_column = pa.array([run_timestamp] * customers_table.num_rows, type=pa.timestamp("us"))
    customers_with_ts = customers_table.append_column(
        "ingestion_timestamp",
        ingestion_column,
    )

    write_start = time.perf_counter()
    write_deltalake(str(output_path), customers_with_ts, mode="overwrite")
    ...
    return BenchmarkExecution(
        ...
        notes=(
            "PyArrow Bronze customers ingest benchmark"
