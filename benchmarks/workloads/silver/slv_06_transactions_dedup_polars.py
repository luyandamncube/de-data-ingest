"""Polars benchmark workload for SLV_06 transactions Silver deduplication."""
...
from benchmarks.workloads.silver.slv_06_transactions_dedup_common import (
    DEDUP_FRACTION,
    active_file_uris,
    build_duplicate_transactions_fixture,
)
...
def run(
    *,
    ...
    """Read duplicated Silver-like transactions Delta, deduplicate, and write Silver."""
    ...
    bronze_path = attempt_root / "silver_like_transactions_with_dups"
    deduped_path = attempt_root / "silver_transactions_deduped"
    shutil.rmtree(attempt_root, ignore_errors=True)
    attempt_root.mkdir(parents=True, exist_ok=True)

    run_timestamp = datetime.now(timezone.utc).replace(microsecond=0, tzinfo=None)
    base_rows, duplicate_rows, rows_in = build_duplicate_transactions_fixture(
    ...
    deduped_df = (
        ...
        .sort(["transaction_id", "ingestion_timestamp"], descending=[False, True])
    ...
    validation_row = (
        ...
        .select(
            ...
            pl.col("transaction_id").null_count().alias("null_transaction_id_count"),
            ...
            pl.col("transaction_time").null_count().alias("null_transaction_time_count"),
    ...
    delta_roundtrip_ok = (
        rows_out == base_rows
        and validation_row["unique_transaction_ids"] == base_rows
        and validation_row["null_transaction_id_count"] == 0
        and validation_row["null_account_id_count"] == 0
        and validation_row["null_transaction_date_count"] == 0
        and validation_row["null_transaction_time_count"] == 0
    ...
    return BenchmarkExecution(
        ...
        notes=(
            "Polars Silver transactions dedup benchmark"
        ...
        artifacts={
            ...
            "null_transaction_id_count": validation_row["null_transaction_id_count"],
            ...
            "null_transaction_time_count": validation_row["null_transaction_time_count"],
